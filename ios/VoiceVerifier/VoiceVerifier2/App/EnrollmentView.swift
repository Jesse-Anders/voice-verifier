...
struct EnrollmentView: View {
        ...
        .fileImporter(isPresented: $showImportTraining, allowedContentTypes: [.audio], allowsMultipleSelection: false) { result in
            ...
            case .success(let urls):
                ...
                do {
                    ...
                    previewURL = dst
                    // Require server embedding for centroid (no local fallback) - do not block UI
    ...
    private func saveProfile() {
        if let centroid = trainedCentroid {
            let files = previewURL != nil ? [previewURL!.lastPathComponent] : nil
            store.add(name: name, centroid: centroid, trainingFiles: files)
            dismiss()
        } else {
            // Fallback: allow saving a profile without embeddings so user can proceed to Recording
            store.add(name: name, centroid: [])
            print("Saved profile without embeddings (model not loaded yet)")
            dismiss()
        }
    }
